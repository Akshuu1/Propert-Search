...
class InfiniteGridMenu {
    ...
    #initTexture() {
        ...
        const cacheKey = this.items.map(item => item.image).join('|');
        if (InfiniteGridMenu.textureCache.has(cacheKey)) {
            const cachedTexture = InfiniteGridMenu.textureCache.get(cacheKey);

            if (gl.isTexture(cachedTexture.texture)) {
                this.tex = cachedTexture.texture;
                this.atlasSize = cachedTexture.atlasSize;
                return;
            } else {
                console.warn('Cached texture does not belong to the current WebGL context. Recreating texture.');
                InfiniteGridMenu.textureCache.delete(cacheKey);
            }
        }
        ...
        const canvas = typeof OffscreenCanvas !== 'undefined'
            ? new OffscreenCanvas(this.atlasSize * cellSize, this.atlasSize * cellSize)
        ...
        const loadImage = async (url, retries = 3) => {
            try {
                ...
                if (retries > 0) {
                    console.warn(`Retrying image load (${retries} retries left): ${url}`);
                    return loadImage(url, retries - 1);
                } else {
                    console.error(`Image load failed after retries: ${url}`, err);
                    return null;
                }
            }
        };
        ...
        Promise.all(
            this.items.map(async (item, index) => {
                ...
                if (!img) {
                    console.warn(`Using placeholder for image at index ${index}`);
                    ...
                    placeholderCtx.fillRect(0, 0, cellSize, cellSize);
                    placeholderCtx.fillStyle = '#000';
                    placeholderCtx.font = '20px Arial';
                    placeholderCtx.fillText('Image Not Found', 10, 50);
                    return placeholder;
                }
                return img;
            })
            ...
            .then(images => {
                ...
                InfiniteGridMenu.textureCache.set(cacheKey, {
                    texture: this.tex,
                    atlasSize: this.atlasSize,
                });
            })
            .catch(err => {
                console.error('Failed to load and process images:', err);
            });
    }
...
export default function InfiniteMenu({ items = [] }) {
    ...
    useEffect(() => {
        ...
        const handleActiveItem = (index) => {
            ...
            if (items[itemIndex]?.link && !items[itemIndex].link.startsWith('http')) {
                router.prefetch(items[itemIndex].link);
            }
        };
    ...
    return (
        <div className="relative w-full h-full">
            <canvas
                ...
                ref={canvasRef}
                className="cursor-grab w-full h-full overflow-hidden relative outline-none active:cursor-grabbing bg-white dark:bg-black"
            />
            ...
            left-[0.5em]
            top-1/2
            transform -translate-y-1/2 translate-x-[20%]
          ...
          `}
            ...
            top-1/2
            right-[-2%]
            transform -translate-y-1/2 translate-x-[-90%]
            transition-all ease-[cubic-bezier(0.25,0.1,0.25,1.0)]
            bg-black
            text-white
            dark:bg-neutral-200
          ...
          `}
                    ...
                    <div className="flex lg:hidden flex-col items-center justify-center mt-30 text-center absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 px-4 w-xs">
                        <h2
                            className={`
            font-black
            text-[1.5rem]
        ...
        `}
                    >
                        <p className="select-none relative text-white dark:text-[#060606] top-[2px] text-[26px]">
                            &#x2197;
                        </p>
                    </div>
                </>
